...
class PositionalEncoding(nn.Module):
    ...
    def create_pe_table(self, d_model, max_len):
        ...
        positons = torch.arange(0, max_len).unsqueeze(1)
        #div_factor = torch.pow(10000, 2 *torch.arange(0, d_model, 2)/d_model)
        div_factor = torch.exp(
        torch.arange(0, d_model, 2, dtype=torch.float32) * (-math.log(10000.0) / d_model)
    )
        pe[:, 0::2] = torch.sin(positons*div_factor)
        pe[:, 1::2] = torch.cos(positons*div_factor)
        pe = pe.unsqueeze(0)

...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.target_embedding       = nn.Embedding(self.num_classes, self.d_model)    # Target embedding
        self.positional_encoding    = PositionalEncoding(self.d_model, self.max_len) # Positional encoding
        self.final_linear           = nn.Linear(self.d_model, self.num_classes) # Final linear layer
        self.dropout                = nn.Dropout(self.drop) # Dropout
        self.norm                   = nn.LayerNorm(d_model) # Layer norm
        self.dec_layers     = nn.ModuleList([SelfAttentionDecoderLayer(self.d_model, self.num_heads, self.d_ff, self.drop) for _ in range(self.num_layers)])
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = nn.ModuleList([SelfAttentionEncoderLayer(d_model=d_model, num_heads=num_encoder_heads, d_ff=d_ff_encoder, dropout=dropout) for _ in range(num_encoder_layers)]) # ModuleList of encoder layers

        # TODO: Create decoder layers
        # Use ModuleList to create a list of decoder layers
        self.dec_layers = nn.ModuleList([CrossAttentionDecoderLayer(d_model=d_model, num_heads=num_decoder_heads, d_ff=d_ff_decoder, dropout=dropout) for _ in range(num_decoder_layers)]) # ModuleList of decoder layers
        ...
        self.ctc_head            = nn.Sequential(self.encoder_norm, nn.Linear(d_model, num_classes), nn.LogSoftmax(dim=-1)) # CTC head
